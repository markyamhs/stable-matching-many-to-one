...
(function () {
        ...
        (function () {
          ...
          if (typeAs[a].matchedTo === undefined) {
            ...
            // otherwise, b already is paired, but it is not to a
              ...
              if (
                typeBs[b].ranking.indexOf(a_) < typeBs[b].ranking.indexOf(a)
              ) {
  ...
  GS.Dom = {
    match: function () {
      ...
      for (var i = 0; i < n; i++) {
        boySelects = GS.Dom.getElementsByClassName(
          typeA + "_rank",
          document.getElementById(typeA + "_" + i)
        );
        var ranking = [];
        ...
        girlSelects = GS.Dom.getElementsByClassName(
          typeB + "_rank",
          document.getElementById(typeB + "_" + i)
        );
        var ranking = [];
      ...
      if (document.getElementById("verbose").checked) {
        ...
        verboseResultsHeader.appendChild(
          document.createTextNode("Verbose Results")
        );
    ...
    setup: function () {
      ...
      if (typeA.length == 0 || typeB.length == 0) {
        ...
        window.alert(
          "The two types (" + typeA + ", " + typeB + ") must be different"
        );
        return;
      }
    ...
    input: function (parent, number, isTypeA) {
      ...
      isTypeA
        ? type.setAttribute("id", typeA + "_" + number)
        : type.setAttribute("id", typeB + "_" + number);
      var name = document.createElement("input");
      name.setAttribute("type", "text");
      isTypeA
        ? name.setAttribute("id", typeA + "_name_" + number)
        : name.setAttribute("id", typeB + "_name_" + number);
      isTypeA
        ? name.setAttribute("value", typeA + number)
        : name.setAttribute("value", typeB + number);
      ...
      for (var rank = 0; rank < n; rank++) {
        ...
        for (var choice = 0; choice < n; choice++) {
          ...
          isTypeA
            ? (option.text = typeB + choice)
            : (option.text = typeA + choice);
          select.add(option, undefined);
        }
        number + rank >= n
          ? (select.selectedIndex = number + rank - n)
          : (select.selectedIndex = number + rank);
        isTypeA
          ? select.setAttribute("id", typeA + number + "_rank_" + rank)
          : select.setAttribute("id", typeB + "_" + number + "_rank_" + rank);
        isTypeA
          ? select.setAttribute(
              "class",
              typeA + "_rank " + typeA + "_rank_" + rank
            )
          : select.setAttribute(
              "class",
              typeB + "_rank " + typeB + "_rank_" + rank
            );
